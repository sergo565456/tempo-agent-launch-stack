...
import { runLocalLiveBoundaryGate } from '../scripts/local-live-boundary-gate.js';

describe('local live-boundary gate', () => {
  it('combines manual blockers and the local autonomous drill without leaking secrets', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'tempo-live-boundary-'));
    ...
    try {
      ...
      const result = await runLocalLiveBoundaryGate({
        agentEnvFile,
        signerEnvFile,
      });
      ...
      assert.equal(result.gates.local_autonomous_drill.ok, true);
      assert.equal(result.gates.local_autonomous_drill.live_actions, false);
      assert(result.next_manual_values.some((action) => action.includes('Agent public HTTPS URL')));
      assert(result.next_manual_values.some((action) => action.includes('Turnkey API-only signer user ID')));
      assert(result.safe_read_only_commands.some((command) => command.includes('turnkey-policy-draft.js')));
      assert(result.safe_read_only_commands.some((command) => command.includes('tempo-access-key-readiness.js')));
      assert(result.safe_dry_run_commands.every((command) => command.includes('-DryRun')));
      assert(result.safe_dry_run_commands.some((command) => command.includes('deploy-vercel-live.ps1')));
      assert(result.safe_dry_run_commands.some((command) => command.includes('deploy-vercel-production.ps1')));
